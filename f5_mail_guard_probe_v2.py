...
def main():
    ...
    p.add_argument("--rate", type=float, help="Target requests/sec; paced sequentially; overrides --delay")
    p.add_argument("--duration", type=float, help="Maximum test duration in seconds")
    ...
    if args.rate is not None:
        print(f"Target rate: {args.rate} req/sec (paced sequential)")
    elif args.delay_sequence:
    ...
    with open(args.csv, "w", newline="", encoding="utf-8") as fh:
        ...
        for i in range(1, args.count + 1):
            runtime_limit = args.duration if args.duration is not None else args.max_runtime
            if runtime_limit and time.monotonic() - start_all >= runtime_limit:
                print("Stopping: duration/runtime limit reached.")
                break
            ...
            if i < args.count:
                if args.rate is not None:
                    ...
                    time.sleep(max(0.0, next_delay - spent))
                else:
